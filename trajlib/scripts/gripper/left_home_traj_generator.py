...
default_Bin_Y = Y_pos;
default_Bin_Z = Z_pos;
from motoman_configuration import arm_left_init, arm_left_side;
...
if __name__=='__main__':
  try:
	...
	print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>";
	print "Planning from Drop to Side";
	arm_left_group.set_joint_value_target(arm_left_side);
	...
	if len(plan.joint_trajectory.points):
		...
		file_name = folder_name + "left_arm_side";
		Save_traj(file_name,plan);
		# Plan from bin to drop
		print "----------------------------------------------";
		print "Planning from Side to Drop";
		arm_left_group.set_joint_value_target(arm_left_init);
		plan = arm_left_group.plan();
		if len(plan.joint_trajectory.points):
			rospy.sleep(5);
			arm_left_group.execute(plan);
			file_name = folder_name + "left_arm_drop";
			Save_traj(file_name,plan);
		else:
